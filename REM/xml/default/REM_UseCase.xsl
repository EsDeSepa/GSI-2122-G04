...
<!-- =========================================== -->
<!-- File    : REM_UseCase.xsl                   -->
<!-- Content : Stylesheet for UseCases           -->
...
<!-- <a> tags now use title attribute and        -->
<!-- header_background variable is used          -->
<!-- =========================================== -->
<!-- Last update: 2004/09/22                     -->
<!-- Action now show comments (if any)           -->
<!-- =========================================== -->
        ...
        <tr>
            <th align="left" valign="top">
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:attribute name="rowspan"><xsl:value-of select="count(rem:step) + 1"/></xsl:attribute>
                    </xsl:otherwise>
                </xsl:choose>
        ...
        <tr>
            <th align="left" valign="top">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:attribute name="rowspan"><xsl:value-of select="count(rem:step/rem:stepException) + 1"/></xsl:attribute>
                    </xsl:otherwise>
                </xsl:choose>
        ...
        <tr>
            <th align="left" valign="top">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:attribute name="rowspan"><xsl:value-of select="count(rem:step/rem:systemAction/rem:performance) + 1"/></xsl:attribute>
                    </xsl:otherwise>
                </xsl:choose>
...
<xsl:template match="rem:actorAction">
    ...
    <a>
        <xsl:attribute name="href">#<xsl:value-of select="@actor"/></xsl:attribute>
        <xsl:attribute name="title"><xsl:value-of select="normalize-space($rem:lang_this_actor_represents)"/>&#32;<xsl:value-of select="normalize-space(id(@actor)/rem:actorRole)"/></xsl:attribute>
        <xsl:value-of select="id(@actor)/rem:name"/> (<xsl:value-of select="@actor"/>)
    </a>
